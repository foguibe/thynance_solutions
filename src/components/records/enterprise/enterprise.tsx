...
export default function EnterpriseRecordsComponent() {
    ...
    const handleAddTransaction = () => {
        const newTransaction: Transaction = { type: "Income", amount: "$1,000", method: "Cash", date: "2025-02-11", description: "Freelance Work", category: "Revenue", currency: "USD", entity: "US Division" };
    ...
    const handleLoadView = (view: any) => {
        ...
        setFilterDate(view.filters.filterDate);
        setFilterVendor(view.filters.filterVendor);
        setFilterClient(view.filters.filterClient);
        setFilterPaymentType(view.filters.filterPaymentType);
        setFilterRegion(view.filters.filterRegion);
        setFilterCurrency(view.filters.filterCurrency);
        setFilterEntity(view.filters.filterEntity);
    ...
    const filteredTransactions = [...transactions].filter(transaction =>
        ...
        (filterCurrency === "" || transaction.currency === filterCurrency) &&
        (filterEntity === "" || transaction.entity === filterEntity)
    ).sort((a, b) => {
        if (sortBy === "amount") {
    ...
    const handleFraudDetection = () => {
        alert("Fraud detection is not implemented in this version.");
    };

    const handleLiquidityRiskAssessment = () => {
            ...
            <div className="container mx-auto">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {filteredTransactions.map((tx, index) => (
                                        <tr key={index} className={index % 2 === 1 ? "bg-gray-100 text-xs" : "text-xs"}>
                                            ...
                                            <td className="px-3 py-1 whitespace-nowrap text-gray-700 border-r border-gray-300">{tx.currency}</td>
                                            <td className="px-3 py-1 whitespace-nowrap text-gray-700 border-r border-gray-300">{tx.entity}</td>
                ...
                <div className="mt-4 bg-white shadow rounded-md p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">Filter by Currency:</label>
                            <select
                                className="border border-gray-300 rounded-md px-3 py-2 text-xs focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="US Division">US Division</option>
                                <option value="EU Division">EU Division</option>
                                <option value="Asia Division">Asia Division</option>
                            </select>
                        </div>
                        <div>
                ...
                <div className="mt-4 bg-white shadow rounded-md p-4">
                    ...
                    >
                        ⚠️ Detect Potential Fraud
                    </button>
                    <button
                        className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold rounded px-3 py-2 transition-colors duration-200 ml-2"
                        onClick={handleLiquidityRiskAssessment}
                    >
